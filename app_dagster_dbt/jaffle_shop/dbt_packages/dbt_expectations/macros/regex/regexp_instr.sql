{% macro regexp_instr(
    source_value, regexp, position=1, occurrence=1, is_raw=False, flags=""
) %}
        ...
        {% if flag not in alphabet %}
            ...
            {{
                exceptions.warn(
                    "flag "
                    ~ flag
                    ~ " not in list of allowed flags for this adapter: "
                    ~ alphabet
                    | join(", ")
                )
            }}
