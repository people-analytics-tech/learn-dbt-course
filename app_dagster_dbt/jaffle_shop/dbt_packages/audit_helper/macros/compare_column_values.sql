{% macro compare_column_values(a_query, b_query, primary_key, column_to_compare) -%}
    {{
        return(
            adapter.dispatch("compare_column_values", "audit_helper")(
...
{% macro default__compare_column_values(
    a_query, b_query, primary_key, column_to_compare
) -%}
    with
        a_query as ({{ a_query }}),
        ...
        joined as (
            select
                ...
                case
                    when
                        a_query.{{ column_to_compare }}
                        = b_query.{{ column_to_compare }}
                    then '✅: perfect match'
                    when
            ...
            full outer join
                b_query on a_query.{{ primary_key }} = b_query.{{ primary_key }}
        ),
