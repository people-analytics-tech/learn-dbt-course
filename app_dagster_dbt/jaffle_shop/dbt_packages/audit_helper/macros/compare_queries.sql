{% macro compare_queries(a_query, b_query, primary_key=None, summarize=true) -%}
    {{
        return(
            adapter.dispatch("compare_queries", "audit_helper")(
                a_query, b_query, primary_key, summarize
            )
        )
    }}
{%- endmacro %}
...
{% macro default__compare_queries(a_query, b_query, primary_key=None, summarize=true) %}
    ...
    with
        ...
        a_intersect_b as (select * from a {{ dbt.intersect() }} select * from b),

        a_except_b as (select * from a {{ dbt.except() }} select * from b),

        b_except_a as (select * from b {{ dbt.except() }} select * from a),
        ...
        {%- else %}
            ...
            final as (
                ...
                order by
                    {{ primary_key ~ ", " if primary_key is not none }} in_a desc,
                    in_b desc
