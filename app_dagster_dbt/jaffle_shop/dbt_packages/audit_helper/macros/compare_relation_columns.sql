{% macro compare_relation_columns(a_relation, b_relation) %}
    {{
        return(
            adapter.dispatch("compare_relation_columns", "audit_helper")(
...
{% macro default__compare_relation_columns(a_relation, b_relation) %}
    ...
    with
        a_cols as ({{ audit_helper.get_columns_in_relation_sql(a_relation) }}),

        b_cols as ({{ audit_helper.get_columns_in_relation_sql(b_relation) }})
